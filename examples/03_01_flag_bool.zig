//! Port of https://github.com/clap-rs/clap/blob/master/examples/tutorial_builder/03_01_flag_bool.rs
...
pub fn cli(a: std.mem.Allocator) clap.Command {
    return clap.Command.init(a, "03_01_flag_bool")
        .about(harness.pkg_about)
        .version(harness.pkg_version)
        .arg(clap.Arg.new("verbose").short('v').long("verbose").action(.set_true));
}
...
pub fn run(a: std.mem.Allocator, argv: []const []const u8, out: *std.ArrayList(u8)) u8 {
    ...
    harness.print(a, out, "verbose: {s}\n", .{if (m.getFlag("verbose")) "true" else "false"});
    return 0;
}
